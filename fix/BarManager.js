...
export default (function (wire, direction, hdirection) {
    ...
    // 合并单元格时有效
    var getColumnIndex = function (list, left) {
        var index
        for (var i = 0, itemLeft; i < list.length; i++) {
            itemLeft = parseInt(list[i].style.left, 10)
            if (itemLeft == left) {
                index = i
                break
            }
        ...
        mutation.get().each(function (target) {
            hoverTable.each(function (table) {
                ...
                getResizer(target, 'data-column').each(function (column) {
                    ...
                    // 原始的坐标
                    var left = Attr.get(target, 'data-initial-left')
                    // 所有纵向resizebar, 合并单元格时需要取最前面的元素
                    var columnsList = target.dom().parentElement.querySelectorAll('[data-column]')
                    var index = getColumnIndex(columnsList, left)
                    Attr.remove(target, 'data-initial-left');
                    events.trigger.adjustWidth(table, delta, isNaN(index) ? parseInt(column, 10) : 0);
                });
